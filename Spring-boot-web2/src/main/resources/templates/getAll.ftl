...
<div class="wrapper">
    <div class="middle">
        ...
        <form action="web/countries" method="post">
        ...
        <#if pageInfo??>
            ...
            <table class="gridtable" style="width:100%;">
                <thead>
                <tr>
                    <th colspan="4">查询结果 - [<a href="web/countries/add">新增国家(地区)</a>]</th>
                </tr>
                <tr>
                    <th>ID</th>
                    <th>国家(地区)名</th>
                ...
                <#list pageInfo.list as country>
                    <tr>
                        ...
                        <td style="text-align:center;">[<a
                                    href="web/countries/view/${country.id}">修改</a>] -
                            [<a href="web/countries/delete/${country.id}">删除</a>]
                        </td>
                    </tr>
                </#list>
                </tbody>
            ...
            <table class="gridtable" style="width:100%;text-align: center;">
                <tr>
                    <#if pageInfo.hasPreviousPage>
                        <td>
                            <a href="web/countries?page=1&rows=${pageInfo.pageSize}&countryname=${queryParam.countryname}&countrycode=${queryParam.countrycode}">首页</a>
                        </td>
                        <td>
                            <a href="web/countries?page=${pageInfo.prePage}&rows=${pageInfo.pageSize}&countryname=${queryParam.countryname}&countrycode=${queryParam.countrycode}">前一页</a>
                        </td>
                    </#if>
                    <#list pageInfo.navigatepageNums as nav>
                        ...
                        <#if nav != pageInfo.pageNum>
                            <td>
                                <a href="web/countries?page=${nav}&rows=${pageInfo.pageSize}&countryname=<#if queryParam.countryname??>${queryParam.countryname}</#if>&countrycode=<#if queryParam.countrycode??>${queryParam.countrycode}</#if>">${nav}</a>
                            </td>
                        </#if>
                    </#list>
                    <#if pageInfo.hasNextPage>
                        <td>
                            <a href="web/countries?page=${pageInfo.nextPage}&rows=${pageInfo.pageSize}&countryname=<#if queryParam.countryname??>${queryParam.countryname}</#if>&countrycode=<#if queryParam.countrycode??>${queryParam.countrycode}</#if>">下一页</a>
                        </td>
                        <td>
                            <a href="web/countries?page=${pageInfo.pages}&rows=${pageInfo.pageSize}&countryname=<#if queryParam.countryname??>${queryParam.countryname}</#if>&countrycode=<#if queryParam.countrycode??>${queryParam.countrycode}</#if>">尾页</a>
                        </td>
                    </#if>
                </tr>
            </table>
